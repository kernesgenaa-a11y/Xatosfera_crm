...
import { errorResponse, extractToken, isRecord, jsonResponse, parseBody, sanitizeUser } from "./utils";
...
export async function createJWT(payload: Record<string, unknown>, secret: string, expiresIn = 3600): Promise<string> {
  const encoder = new TextEncoder();
...
export async function verifyJWT(token: string, secret: string): Promise<Record<string, unknown> | null> {
  try {
    ...
    if (!isRecord(payload) || typeof payload.exp !== "number" || payload.exp < Math.floor(Date.now() / 1000)) return null;
    return payload;
  } catch {
    return null;
  }
...
export async function getUserById(env: Env, userId: string, columns = USER_AUTH_COLUMNS): Promise<UserRecord | null> {
  return (await env.DB.prepare(`SELECT ${columns} FROM users WHERE id = ?`).bind(userId).first<UserRecord>()) ?? null;
}

export async function handleRegister(request: Request, env: Env, jwtSecret: string): Promise<Response> {
  ...
  if (!body || typeof body.email !== "string" || typeof body.password !== "string" || typeof body.full_name !== "string") {
  ...
  if (body.password.length < 8) return errorResponse("Password must be at least 8 characters", 400, env, request);
  ...
  await env.DB.prepare("INSERT INTO sessions (id, user_id, refresh_token, expires_at, created_at) VALUES (?, ?, ?, ?, datetime('now'))")
...
export async function handleLogin(request: Request, env: Env, jwtSecret: string): Promise<Response> {
  const rateLimit = await checkRateLimit(request, env, {
  ...
  const user = await env.DB.prepare(`SELECT ${USER_AUTH_COLUMNS} FROM users WHERE email = ?`).bind(body.email).first<UserRecord>();
...
export async function handleRefresh(request: Request, env: Env, jwtSecret: string): Promise<Response> {
  const body = await parseBody(request);
  if (!body || typeof body.refresh_token !== "string") return errorResponse("Missing refresh token", 400, env, request);
...
export async function handleGetMe(request: Request, env: Env, jwtSecret: string): Promise<Response> {
  const token = extractToken(request);
  if (!token) return errorResponse("No token provided", 401, env, request);
  const payload = await verifyJWT(token, jwtSecret);
  if (!payload || typeof payload.sub !== "string") return errorResponse("Invalid token", 401, env, request);
...
export async function verifyAuth(request: Request, env: Env, jwtSecret: string): Promise<AuthResult> {
  const token = extractToken(request);
