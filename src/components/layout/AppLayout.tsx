...
import bgImage from "@/assets/112.png";
...
export const AppLayout = ({ children }: AppLayoutProps) => {
  ...
  const openPanel = () => { refresh(); setNotifOpen(o => !o); };
  ...
  const handleNotifClick = async (n: typeof notifications[0]) => {
    if (!Boolean(n.is_read)) await markRead(n.id);
    setNotifOpen(false);
  ...
  return (
    ...
    />
      ...
      <main id="main-scroll" className="flex-1 overflow-y-auto overflow-x-hidden relative mt-16 lg:mt-0">
          ...
          <div className="absolute top-[64px] right-4 lg:top-[56px] lg:right-6 w-80 max-h-[70vh] bg-card border rounded-xl shadow-xl flex flex-col overflow-hidden animate-in slide-in-from-top-2 fade-in duration-200">
            ...
            <div className="flex items-center justify-between px-4 py-3 border-b bg-muted/30">
              ...
              <div className="flex items-center gap-1">
                {unreadCount > 0 && (
                  <button onClick={markAllRead} className="p-1.5 rounded-md hover:bg-muted transition-colors" title="Позначити всі як прочитані">
                    <CheckCheck className="h-3.5 w-3.5 text-muted-foreground" />
                  </button>
                )}
                <button onClick={() => setNotifOpen(false)} className="p-1.5 rounded-md hover:bg-muted transition-colors">
            ...
            <div className="overflow-y-auto flex-1">
              {notifications.length === 0 ? (
                ...
                const iconColor = n.type === 'assignment' ? 'text-blue-600 bg-blue-50' : n.message.includes('Виконано') ? 'text-green-600 bg-green-50' : n.message.includes('виконано') ? 'text-red-500 bg-red-50' : 'text-primary bg-primary/10';
                return (
                  <div
                    key={n.id}
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-1">
                        <p className={`text-sm leading-snug ${unread ? 'font-semibold' : 'font-medium'}`}>{n.title}</p>
